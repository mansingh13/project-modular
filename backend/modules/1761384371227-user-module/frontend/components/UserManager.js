...
const UserManager = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/modules/MODULE_PATH/users');
      if (response.ok) {
  ...
  const updateUser = async (id, updates) => {
    try {
      const response = await fetch(`http://localhost:3001/api/modules/MODULE_PATH/users/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
