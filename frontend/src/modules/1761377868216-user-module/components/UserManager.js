...
const UserManager = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/modules/todo-manager/users');
      if (response.ok) {
  ...
  const updateUser = async (id, updates) => {
    try {
      const response = await fetch(`http://localhost:3001/api/modules/todo-manager/users/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
