...
const TodoManager = () => {
  ...
  const fetchTodos = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/modules/1761375941497-updated-todo-module/todos');
      if (response.ok) {
  ...
  const updateTodo = async (id, updates) => {
    try {
      const response = await fetch(`http://localhost:3001/api/modules/1761375941497-updated-todo-module/todos/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
