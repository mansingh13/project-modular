...
const ModuleLoader = ({ modules, onModuleUploaded }) => {
  ...
  const toggleModule = async (moduleId) => {
    try {
      const response = await fetch(`http://localhost:3001/api/modules/${moduleId}/toggle`, {
        method: 'PUT',
      });
        ...
        {modules.length === 0 ? (
          ...
          <div className="modules-grid">
            {modules.map((module, index) => (
              <div key={module.id || index} className="module-card" style={{
                opacity: module.active === false ? 0.6 : 1,
                backgroundColor: module.active === false ? '#f8f9fa' : 'white'
              }}>
                <h4>{module.name} {module.active === false && <span style={{color: '#dc3545', fontSize: '12px'}}>(Inactive)</span>}</h4>
                ...
                <div style={{ marginTop: '10px' }}>
                  <button
                    ...
                    style={{
                      padding: '6px 12px',
                      backgroundColor: module.active === false ? '#28a745' : '#ffc107',
                      color: 'white',
                      border: 'none',
                      borderRadius: '4px',
                      cursor: 'pointer',
                  ...
                  >
                    {module.active === false ? 'Activate' : 'Deactivate'}
                  </button>
                  <button
