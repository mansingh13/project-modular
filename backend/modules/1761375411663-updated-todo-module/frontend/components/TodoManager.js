...
const TodoManager = () => {
  ...
  const fetchTodos = async () => {
    try {
      ...
      const moduleName = currentPath.split('/').pop() || '1761371865208-todo-module';
      const response = await fetch(`http://localhost:3001/api/modules/${moduleName}/todos`);
      if (response.ok) {
  ...
  const updateTodo = async (id, updates) => {
    try {
      ...
      const currentPath = window.location.pathname;
      const moduleName = currentPath.split('/').pop() || '1761371865208-todo-module';
      const response = await fetch(`http://localhost:3001/api/modules/${moduleName}/todos/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
